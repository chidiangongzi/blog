...
#                    third[it[(np.where(iw[0][:] == i)[0][0])]:nt+1, i].max())
            ...
            for j in range(it[(np.where(iw[0][:] == i)[0][0])], nt):
                ...
                if np.any(d[j, i] > 3.3) and np.any(d[j, i] <= 3.6):
                    cc = '#CD1076' #deep red
                if (j%5==0 and j<=nt-3):
                    cwind = ax.quiver(second[j+1, i],third[j+1, i],vv[j+1, i],ww[j+1, i],color='#9900ff',width=0.0060,scale=60,alpha=1)
                figure1 = ax.plot(second[j:j + 2, i],
                                  third[j:j + 2, i],
                                  color=cc,
                                  lw=2.0)
                j += 1
            kdiw += 1
            ax.quiverkey(cwind,0.81,0.91,5,"5m·$\mathregular{{s^{-1}}}$",labelpos='E',coordinates='axes',fontproperties={'size':20,'family':'Times New Roman'})
        ...
        #    plt.xlim(minlon, maxlon+0.002,0.05)
            plt.xlim(minlat, maxlat+0.002,0.05)
        ...
        #    ax.set_xlim(minlon, maxlon+0.002)
            ...
            if np.any((gyglat)==1):
                print("cnm,要改",minlat)
                maxlat = float(int(math.ceil((maxlat-0.0002) * 100 / 2) *2)/100)
                minlat = float(int(math.floor((minlat+0.0002) * 100 / 2) *2)/100)
                print("cnm,要改",float(int(math.floor(minlat * 100 / 2) *2)/100))
                x_major_locator = MultipleLocator(0.02)
                ax.xaxis.set_major_locator(x_major_locator)
                plt.xlim(minlat, maxlat+0.002,0.02)
                ax.set_xlim(minlat, maxlat+0.002)
                print("cnm,要改")
        ...
        #    plt.subplots_adjust(left=0.05, bottom=0.1)
            ...
            plt.savefig("./kmeans-latitude-K=" + str(k+1)+ "-"+ str(kdiw)+".png", dpi=500)
            plt.close()
