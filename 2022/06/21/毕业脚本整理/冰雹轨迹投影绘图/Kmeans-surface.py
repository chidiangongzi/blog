...
#                    third[it[(np.where(iw[0][:] == i)[0][0])]:nt+1, i].max())
            ...
            for j in range(it[(np.where(iw[0][:] == i)[0][0])], nt):
                ...
                if np.any(d[j, i] > 3.3) and np.any(d[j, i] <= 3.6):
                    cc = '#CD1076' #deep red
                if (j%5==0 and j<=nt-3):
                    cwind = ax.quiver(first[j+1, i],second[j+1, i],uu[j+1, i],vv[j+1, i],color='#9900ff',width=0.0060,scale=60,alpha=1)
...
#            print("minlat", float(int(math.floor(minlat * 100 / 5) *5)/100))
        ...
        #    plt.xlim(minlon, maxlon+0.002,0.05)
            plt.xlim(minlon, maxlon+0.002,0.05)
            plt.ylim(minlat, maxlat+0.002,0.05)
        ...
        #    plt.yticks(rotation=40+20)
            ...
            ax.set_ylim(minlat, maxlat+0.002)
            ...
            if np.any((gyglat)==1):
                ...
                ax.yaxis.set_major_locator(y_major_locator)
                plt.ylim(minlat, maxlat+0.002,0.02)
                ax.set_ylim(minlat, maxlat+0.002)
                print("cnm,要改")
                print("cnm,要改",minlat)
            if np.any((gyglon)==1):
        ...
        #    plt.subplots_adjust(left=0.05, bottom=0.1)
            ...
            plt.savefig("./kmeans-surface-K=" + str(k+1)+ "-"+ str(kdiw)+".png", dpi=500)
            plt.close()
