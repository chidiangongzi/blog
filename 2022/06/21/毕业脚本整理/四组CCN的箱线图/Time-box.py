...
for k in range(1):
    ...
    for nd in range(5):
        ### 每个粒径段每一条轨迹的时间序列筛选
        timell = [0 for x in range(0, len(diw[nd][0]))]
        for ll in range(len(diw[nd][0])):
            # print("每一条轨迹的矩阵编号", diw[nd][0][ll])
            # 最晚增长，最早触地时刻作为最终时刻判断
            nzz = np.array(third_2000[:, diw[nd][0][ll]].min()) ### 最底层
            nt0 = max(max(np.where(d[:, diw[nd][0][ll]] <= 0.5)))
            ntt = max(
                max(np.where(dlt[:, diw[nd][0][ll]] > 0.0)[0]),
                min(np.where(third_2000[:, diw[nd][0][ll]] <= nzz)[0]))
            itt = int((ntt + 2) * 50 / 60) # 多
            it0 = int((nt0) * 50 / 60) #
            #print("增长时间=", itt - it0, "min")
            timell[ll] = (itt - it0)
            timek1d[nd] = timell
...
ax.spines['right'].set_linewidth(0)
ax.spines['bottom'].set_linewidth(bwith)
plt.ylabel('Hail Diameter  (cm)',
           fontsize=18)
plt.ylabel('Time  (min)',

           fontsize=18)

plt.savefig(fname="Time-box.png")
