...
QNG = np.empty([360, 360, 35], dtype=float)
...
QNG[:, :, :] = 0
for i in range(nsc):
    QNG[int(second[int(SC[i])]),
        int(first[int(SC[i])]),
        int(third[int(SC[i])])] = qng[int(SC[i])]

WW[:, :, :] = 0
for i in range(nn):
    WW[int(second[i]), int(first[i]), int(third[i])] = w[i]

q2d = np.amax(QNG, axis=2)
...
levels = [0, 1000, 5000, 10000, 100000, 500000, 1000000]
...
im = plt.contourf(
    ...
    q2d,
    8,
    colors=["#1aad19", "#00ff33", "#ffff00", "#ff9933", "red", "#cc00ff"],
    levels=levels[:])
#    extend='both')

iw = np.where((q2d[:, :] >= 1000) & (q2d[:, :] <= 5000))
iw0 = np.array(iw[0])
iw1 = np.array(iw[1])
#print(iw0)
#print(iw1)
...
ax.legend(fontsize=11,
          loc='lower center',
          bbox_to_anchor=(0.10, 1.02, 0.5, 0.5),
          frameon=True,
          fancybox=True,
          framealpha=0.2,
          borderpad=0.3,
...
plt.savefig("./dzx-qg.png", dpi=500)
#plt.show()
